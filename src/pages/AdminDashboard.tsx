...
const AdminDashboard = () => {
  ...
  const fetchUsersWithRoles = async () => {
    ...
    const usersWithRoles = profilesData.map(profile => {
      const userRole = usersData?.find(u => u.email === profile.email)?.role || 'Client';
      return {
        ...profile,
        user_role: userRole
      };
              ...
              <div className="overflow-x-auto">
                <table className="w-full border border-sage/50 divide-y divide-sage/50">
                  <thead className="bg-sage/20">
                    <tr>
                      <th className="px-4 py-2 text-left text-sm font-medium text-forest-green">User</th>
                      <th className="px-4 py-2 text-center text-sm font-medium text-forest-green">Role</th>
                      <th className="px-4 py-2 text-center text-sm font-medium text-forest-green">Actions</th>
                    </tr>
                  </thead>
                    ...
                    {users.map(user => (
                      <tr key={user.id} className="hover:bg-sage/10">
                        ...
                        <td className="px-4 py-2 text-center">
                          <select
                            value={user.user_role || 'Client'}
                            onChange={(e) => updateUserRole(user.email, e.target.value as 'Client' | 'Admin')}
                            className="bg-white border border-sage rounded px-2 py-1 text-sm"
                          >
