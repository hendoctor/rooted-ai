-- Fix company role display for pending invitations in unified user data
...
BEGIN
  ...
  WITH all_user_data AS (
    ...
    -- Pending invitations (not yet registered) - FIXED to use actual company_role
    SELECT 
      ui.email,
        ...
        WHEN ui.client_name IS NOT NULL THEN
          jsonb_build_array(
            jsonb_build_object(
              ...
              'name', ui.client_name,
              'slug', c.slug,
              'userRole', COALESCE(ui.company_role, 'Member')  -- Fixed: use actual company_role instead of hardcoded 'Member'
            )
          )
        ELSE '[]'::jsonb
      END as companies,
