-- Update cleanup function to delete dependent newsletter subscriptions before deleting users
...
BEGIN
  ...
  -- 4. Delete newsletter subscriptions to avoid FK constraint failures
  DELETE FROM public.newsletter_subscriptions 
  WHERE email = user_email;
  GET DIAGNOSTICS deleted_count = ROW_COUNT;
  cleanup_summary := jsonb_set(cleanup_summary, '{newsletter_subscriptions}', to_jsonb(deleted_count));
