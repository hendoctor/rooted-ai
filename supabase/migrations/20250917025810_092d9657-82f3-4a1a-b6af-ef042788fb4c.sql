-- Update delete_user_completely_enhanced to hard delete ALL invitations and improve cleanup
...
BEGIN
  ...
  IF target_user_record.id IS NULL THEN
    -- If no users row exists, still remove newsletter entries and invitations to fully erase access
    ...
    -- Hard delete ALL invitations for this email
    DELETE FROM public.user_invitations WHERE lower(trim(email)) = normalized_email;
    GET DIAGNOSTICS deleted_count = ROW_COUNT;
    cleanup_summary := jsonb_set(cleanup_summary, '{invitations_deleted}', to_jsonb(deleted_count));
  ...
  -- 2. Hard delete ALL invitations (not just cancel) - case-insensitive and normalized
  DELETE FROM public.user_invitations 
  WHERE lower(trim(email)) = normalized_email;
  GET DIAGNOSTICS deleted_count = ROW_COUNT;
  cleanup_summary := jsonb_set(cleanup_summary, '{invitations_deleted}', to_jsonb(deleted_count));

  -- 3. Delete company memberships
  ...
  -- 5. Update security audit logs (don't delete for audit trail, but anonymize)
