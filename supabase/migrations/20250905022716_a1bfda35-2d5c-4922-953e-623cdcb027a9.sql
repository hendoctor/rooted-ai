...
-- Add additional security logging for invitation access attempts
CREATE OR REPLACE FUNCTION public.log_invitation_access_attempt()
RETURNS TRIGGER AS $$
BEGIN
  -- Log all invitation access attempts for monitoring
...
DROP TRIGGER IF EXISTS invitation_access_logging ON public.user_invitations;
CREATE TRIGGER invitation_access_logging
