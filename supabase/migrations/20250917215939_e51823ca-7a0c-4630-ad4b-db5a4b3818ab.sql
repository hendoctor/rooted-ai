-- Fix get_user_context_optimized to properly set is_admin based on company role
CREATE OR REPLACE FUNCTION public.get_user_context_optimized(p_user_id uuid)
  ...
  user_companies AS (
    SELECT jsonb_agg(
      jsonb_build_object(
        ...
        'company_slug', c.slug,
        'user_role', cm.role,
        'is_admin', (cm.role = 'Admin' OR (SELECT user_role = 'Admin' FROM user_data))
      )
    ) as companies_data
    FROM companies c
