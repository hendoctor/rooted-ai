-- Idempotent (re)create notification trigger function and triggers
DO $$ BEGIN
  PERFORM 1 FROM pg_proc WHERE proname = 'create_content_notifications';
  IF FOUND THEN
    DROP FUNCTION public.create_content_notifications() CASCADE;
  END IF;
END $$;
...
-- Ensure realtime is configured (idempotent)
